/* gcc -O2  omp_factorize.c -o omp_factorize   -lcrypto -lm -fopenmp */

/*
./omp_factorize -m B81915BC0A2222F4B  -a 4    # modulus 68 bit - prime 34 bit (4 addr + 30 block) 
*/
...
int main(int argc, char *argv[])
{
   ...
   if (nt) omp_set_num_threads(nt);
    ...
    printf(" %lu bits - prime: address %lu bit, block %lu bit,  num_threads %d\n\n", modulus_bit, block_addr_bit, block_dim_bit, omp_get_max_threads() );
    fflush(stdout);
    ...
    for (block_idx=block_addr_size-1; block_idx > -1 ; block_idx--)
         {
         if (found)   continue; 
         printf("thread %d  processing block %d\n", omp_get_thread_num(), block_idx);
         t1=omp_get_wtime();
         found=block_factorize (block_idx);
         t2=omp_get_wtime();
         printf("thread %d  processed block %d in %.1f sec. with result %d \n", omp_get_thread_num(), block_idx, t2-t1, found);
        }
    printf("FOUND: ");
...
void usage(char * argv[])
{
  printf ("\n%s [-m modulus] [-a block_addr_bit] [-t num_threads] [-g modulus_bit] [-h]\n",argv[0]);
