...
int main(int argc, char **argv)
{
   ...
   for(iter=0; iter<MAX_ITER; iter++)
   {
      ...
      if (iter % 100 == 0)
      {
         printf("MPI_Gather from %d, at iteration %d\n", mpi_rank, iter);
         ...
         if (mpi_rank == 0)
         {
            printf("Snapshot of iteration %d using save_heatmap -> heatmap.txt\n", iter);
   ...
   if (mpi_rank == 0)
   {
      ...
      printf("final gather from proccess %d\n", mpi_rank);
   }
...
void save_heatmap(float *h_T, const int NX, const int NY, int iter)
{
   ...
   fprintf(fp,"ITERATION: %d and Total Task: %d\n", iter, mpi_size);
...
void options(int argc, char *argv[])
{
   ...
   while((c=getopt(argc,argv,"c:r:s:dh"))!=-1)
   {
      switch(c)
      {
         case 'c': WNX=atoi(optarg); break;
         case 'r': WNY=atoi(optarg); break;
         case 's': MAX_ITER=atoi(optarg); break;
