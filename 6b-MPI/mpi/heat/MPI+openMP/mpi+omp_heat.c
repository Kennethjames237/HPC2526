...
OMP_NUM_THREADS=4  mpirun -npernode 1  mpi+omp_heat -r 32 -c 32 -s 4
*/
...
int MAX_ITER = 100;   // --- Number of Jacobi iterations
int NT=0; 
int NX,NY;
...
int main(int argc, char **argv)
{
  ...
  if (help) { 
       if (mpi_rank==0) printf ("\n%s [-t ThreadNum] [-c ncols] [-r nrows] [-s nsteps] [-d] [-h] \n\n",argv[0]); 
       MPI_Finalize(); 
      return (0);
   }
...
// scambio delle righe di bordo
  ...
  if(mpi_rank == 0) {
    fprintf(stderr,"MPI+OMP, %d, %d, %d,  %d, %d, %f\n", mpi_size, omp_get_max_threads(),   WNX, WNY, MAX_ITER, t2-t1);
    fflush(stderr);
   }
...
void options(int argc, char * argv[]) {
  ...
  int i;
   while ( (i = getopt(argc, argv, "t:c:r:s:dh")) != -1) {
        switch (i) {
